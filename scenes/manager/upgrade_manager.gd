...
var upgrade_scepter = preload("res://resources/upgrades/scepter.tres")
...
func _ready():
	...
	upgrade_pool.add_item(upgrade_scepter, 10)
...
func pick_upgrades():
	...
	for i in 2:
		if upgrade_pool.items.size() == choosen_upgrades.size():
			break
		var chosen_upgrade = upgrade_pool.pick_item(choosen_upgrades)
...
func on_level_up(current_level: int):
	var upgrade_screen_scene = upgrade_screen_scene.instantiate()
	add_child(upgrade_screen_scene)
	var chosen_upgrades = pick_upgrades()
	upgrade_screen_scene.set_ability_upgrade(chosen_upgrades as Array[AbilityUpgrade])
	upgrade_screen_scene.upgrade_selected.connect(on_upgrade_selected)
